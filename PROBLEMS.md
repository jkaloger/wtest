...
- Sandbox denies binding a unix socket inside the repo (`listen unix ...: operation not permitted`), so process-compose's control channel uses TCP on loopback (`PC_PORT`, default 8474). `just server -D` / `just server-down` work for the agent.
- Sandbox denies `curl` invocations written directly in the agent's shell (both to :3000 and :4010), though `curl` inside a `just` recipe runs fine. Used node `fetch` one-liners for ad-hoc probes.
- Agent cannot read or write `.env.test` through the Read/Edit tools, and bash commands whose text mentions `.env.test` are denied. Consequence: `APP_PORT` defaults live in the justfile, not the dotenv file, and process-compose gets env via `just`'s dotenv-load rather than `-e`.
